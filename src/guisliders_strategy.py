...
class PlotGUI():
    def __init__(self, root, colorpalette = 'light') -> None:
        ...
        # Location of the frames for a symmetric layout
        # top left x-coord (from left) starting point of the left frame
        self.left_relx = 0.02

        # top left y-coord (from top) starting point of the left frame
        self.left_rely = 0.02

        # x-length of the left frame
        ...
        # tk Text widget default width (title & descriptions)
        ...
        self.strat_options = ("Custom strategy",
                              ...
                              "Bottom Butterfly",
                              "Top Iron Condor",
                              "Bottom Iron Condor")  #  I hate these top/bottom names, rename to long/short.

        # chosen strategy (custom or predefined)
        self.chosen_strategy = None

        # flag: true is the "add option" button is pressed, False otherwise
        self.flag_add_option = False

        # flag: True is the "Reset strategy" button is pressed, False otherwise
        self.flag_reset_strategy = False

        # Dicts
        # Dict of options data for a custom strat
        self.CusOptData = dict()

        # dict of tk.Entries values of the custom strat
        self.Entries = dict()

        # dict of tk.labels for option prices of the custom strat
        self.LabPrice = dict()


        # default payoff plot colors for the custom strat
        # max colors = 5 = max number of options
        self.optioncolors = ["tab:blue", "tab:red", "forestgreen", "sandybrown", "chocolate", "gray"]
    ...
    def left_frame(self):
        ...
        # create left frame and place it in the gui
        ...
        # set initial row and col values
        self.gridrow = 0
        self.gridcol = 0
        ...
        self.config_tktext(self.titleframe, textmsg = self.title_lframe, textrow = self.gridrow, textpady = (20, 0))
    ...
    def right_frame(self):
        ...
        # subplot spaces
        plt.subplots_adjust(left = 0.12, right = 0.96, top = 0.96, bottom = 0.16, hspace = 0.27, wspace = 0.3)
