...
class PlotGUI:
    def __init__(self, root) -> None:
        ...
        self.pic2 = tk.Label(master = self.framel,
                             ...
                             relief = self.descrelief,
                             width = 15)
        self.pic2.grid(columnspan = 2, padx=20, pady=(10, 2), sticky="nsew")
        ...
        self.slider_r_ax = plt.axes([0.3, 0.08, 0.50, 0.015])
        self.slider_v_ax = plt.axes([0.3, 0.08, 0.50, 0.015])
...
where
        ...
        # plot the title
        plt.suptitle("BSM Options Pricing: {} Option".format("Call" if self.CP == "C" else "Put"), fontsize = 15, fontweight = 'bold', color = "k")
        ...
        def setlegend(self):
            ...
            atm = self.Sset[idx]
            self.atmp = []
            self.atmp.append(self.ax[0].scatter(atm, self.prices[idx],  c = 'k', s=10, marker='o', label = "ATM Price ({:.0f})".format(round(self.prices[idx], 2))))
            self.atmp.append(self.ax[1].scatter(atm, self.lambdas[idx], c = 'k', s=10, marker='o', label = "ATM Lambda ({:.2f})".format(round(self.lambdas[idx], 2))))
            ...
            self.atmp.append(self.ax[3].scatter(atm, self.gammas[idx],  c = 'k', s=10, marker='o', label = "ATM Gamma ({:.2f})".format(round(self.gammas[idx], 2))))
            self.atmp.append(self.ax[4].scatter(atm, self.thetas[idx],  c = 'k', s=10, marker='o', label = "ATM Theta ({:.2f})".format(round(self.thetas[idx], 2))))
            self.atmp.append(self.ax[5].scatter(atm, self.vegas[idx],   c = 'k', s=10, marker='o', label = "ATM Vega ({:.2f})".format(round(self.vegas[idx], 2))))
        ...
        def onslide(self, val):
            ...
            self.option = [BSOpt(self.CP, s, self.K, current_T, current_r/100, current_v/100, q = self.q) for s in self.Sset]

            # Get prices and greeks for all set of underlyings for new values of the sliders
            self.prices = [o.price()        for o in self.option]
            self.lambdas = [o.lambda_()     for o in self.option]
            self.deltas = [o.delta()        for o in self.option]
            self.gammas = [o.gamma()*100    for o in self.option]
            self.thetas = [o.theta()        for o in self.option]
            self.vegas = [o.vega()          for o in self.option]
            ...
            if self.CP =="C":
                if current_T != 0:
                    M = self.lambdas[ min(np.where(np.array(self.lambdas) < np.inf)[0]) ] # possibly abs(np.inf()) or +np.inf()
            ...
            if current_T != 0:
                ...
                self.ax[4].set_ylim(top    = max(self.thetas) - max(self.thetas)     if max(self.thetas) < 0 else max(self.thetas) + max(self.thetas))
            else:
                self.ax[4].set_ylim([-5, 1])

            # Vega
            if current_T != 0:
                self.ax[4].set_ylim([min(self.vegas) - 0.1*max(self.vegas), max(self.vegas) + 0.1*max(self.vegas)])
